...
- **Project docs**: Always includes the root `CLAUDE.md` and `.claude/CLAUDE.md`; nested `<dir>/CLAUDE.md` files (any depth, e.g. `apps/*/CLAUDE.md`, `engines/*/CLAUDE.md`) are discovered and included only when a changed file lives under that directory. Up to 20 files, 64KB each, 128KB total.
